...
def _scaled_specs(
    ...
    *,
    ...
    if target_total is not None and target_total <= 0:
        raise ValueError("target_total must be > 0 when provided.")
    ...
    if target_total is None:
        return [
            CategorySpec(
                ...
                n=max(1, int(round(spec.n * scale_factor))),
                needs_ground_truth=spec.needs_ground_truth,
            )
            for spec in specs
        ]

    base_total = sum(spec.n for spec in specs)
    raw_targets = [target_total * (spec.n / base_total) for spec in specs]
    floors = [int(x) for x in raw_targets]
    remainder = int(target_total - sum(floors))
    frac_order = sorted(
        range(len(specs)),
        key=lambda i: (raw_targets[i] - floors[i]),
        reverse=True,
    )
    allocations = floors[:]
    for i in range(remainder):
        allocations[frac_order[i % len(specs)]] += 1

    scaled: list[CategorySpec] = []
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Scale category counts uniformly (ignored if --target-per-trait is set).",
    )
    parser.add_argument(
        "--target-per-trait",
        type=int,
        default=0,
        help="If >0, rescale each trait to exactly this many rows while preserving category mix.",
    )
    parser.add_argument(
        "--dry-run-plan",
        action="store_true",
        help="Emit a generation plan only; do not call the API or write prompt files.",
    )
    ...
    if args.target_per_trait < 0:
        raise ValueError("--target-per-trait must be >= 0.")

    target_total = int(args.target_per_trait) if args.target_per_trait > 0 else None
    scaled_specs_by_trait: dict[str, list[CategorySpec]] = {
        trait: _scaled_specs(
            HELDOUT_CATEGORIES[trait],
    ...
    if args.dry_run_plan:
        plan = {
            ...
            "dry_run": True,
            "scale_factor": float(args.scale_factor),
            "target_per_trait": target_total,
            "traits": {
                trait: {
                    "total": int(sum(spec.n for spec in specs)),
                    "categories": {spec.category: spec.n for spec in specs},
                }
                for trait, specs in scaled_specs_by_trait.items()
            },
        }
        plan_path = SCRATCH_DIR / "week2_heldout_prompt_generation_plan.json"
    ...
    for trait in args.traits:
        out_path, audit = generate_trait_heldout_dataset(
            client=client,
            trait=trait,
            specs=scaled_specs_by_trait[trait],
        )
