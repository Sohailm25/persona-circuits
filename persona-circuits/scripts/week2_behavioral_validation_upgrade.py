...
try:
    from scripts.shared.trait_rubrics import JUDGE_PROMPT_HEADER as SHARED_JUDGE_PROMPT_HEADER
    from scripts.shared.trait_rubrics import RUBRICS as SHARED_RUBRICS
...
DEFAULT_WEEK2_UPGRADE_MODAL_TIMEOUT_HOURS = 20
DEFAULT_PROGRESS_LOG_EVERY_ROWS = 5
DEFAULT_PROGRESS_LOG_EVERY_COMBOS = 1
DEFAULT_PROGRESS_MIN_INTERVAL_SECONDS = 20.0
ALLOWED_COMBO_SELECTION_POLICIES = {"smallest_feasible_alpha", "max_bidirectional_effect"}
ALLOWED_COHERENCE_GATE_MODES = {"absolute_and_relative", "relative_only", "absolute_only"}
...
CHECKPOINT_VOLUME_DIR = Path("/models/persona-circuits/week2/checkpoints")
...
RUBRICS = dict(SHARED_RUBRICS)

JUDGE_PROMPT_HEADER = SHARED_JUDGE_PROMPT_HEADER


def _load_week2_upgrade_runtime_settings() -> dict[str, Any]:
    if not EXPERIMENT_CONFIG_PATH.exists():
        return {}
    try:
        raw = yaml.safe_load(EXPERIMENT_CONFIG_PATH.read_text(encoding="utf-8")) or {}
    except Exception:  # noqa: BLE001
        return {}
    runtime = raw.get("runtime", {})
    if not isinstance(runtime, dict):
        return {}
    stage_cfg = runtime.get("week2_behavioral_validation_upgrade", {})
    return stage_cfg if isinstance(stage_cfg, dict) else {}


_RUNTIME_SETTINGS = _load_week2_upgrade_runtime_settings()
WEEK2_UPGRADE_MODAL_TIMEOUT_HOURS = max(
    1,
    int(
        os.getenv(
            "WEEK2_UPGRADE_MODAL_TIMEOUT_HOURS",
            str(_RUNTIME_SETTINGS.get("modal_timeout_hours", DEFAULT_WEEK2_UPGRADE_MODAL_TIMEOUT_HOURS)),
        )
    ),
)
WEEK2_UPGRADE_PROGRESS_LOG_EVERY_ROWS = max(
    1,
    int(_RUNTIME_SETTINGS.get("progress_log_every_rows", DEFAULT_PROGRESS_LOG_EVERY_ROWS)),
)
WEEK2_UPGRADE_PROGRESS_LOG_EVERY_COMBOS = max(
    1,
    int(_RUNTIME_SETTINGS.get("progress_log_every_combos", DEFAULT_PROGRESS_LOG_EVERY_COMBOS)),
)
WEEK2_UPGRADE_PROGRESS_MIN_INTERVAL_SECONDS = max(
    0.0,
    float(_RUNTIME_SETTINGS.get("progress_min_interval_seconds", DEFAULT_PROGRESS_MIN_INTERVAL_SECONDS)),
)
...
def _set_modal_cache_env() -> None:
    ...
    CHECKPOINT_VOLUME_DIR.mkdir(parents=True, exist_ok=True)
...
def _decode_int_key_dict(raw: Any) -> dict[int, Any]:
    out: dict[int, Any] = {}
...
def _combo_key(layer: int, alpha: float) -> str:
    return f"l{int(layer)}_a{float(alpha):.8f}"


def _resolve_checkpoint_key(*, trait: str, seed: int, run_name: str | None, checkpoint_key: str | None) -> str:
    if checkpoint_key and checkpoint_key.strip():
        candidate = checkpoint_key.strip()
    elif run_name and run_name.strip():
        candidate = run_name.strip()
    else:
        candidate = f"week2-upgrade-{trait}-s{seed}"
    normalized = re.sub(r"[^a-zA-Z0-9_.-]+", "-", candidate)
    return normalized.strip("-") or f"week2-upgrade-{trait}-s{seed}"
...
def _alpha_grid_from_config(config: dict[str, Any]) -> list[float]:
    raw_values = config.get("steering", {}).get("coefficients")
    if not isinstance(raw_values, list) or not raw_values:
        raise ValueError("Config steering.coefficients must be a non-empty list.")
    out = [float(x) for x in raw_values]
    if any(not np.isfinite(x) for x in out):
        raise ValueError("Config steering.coefficients contains non-finite values.")
    return out
...
def _combo_selection_policy_from_config(config: dict[str, Any]) -> str:
    steering_cfg = config.get("steering", {})
    if not isinstance(steering_cfg, dict):
        steering_cfg = {}
    policy = str(steering_cfg.get("combo_selection_policy", "smallest_feasible_alpha")).strip().lower()
    if policy not in ALLOWED_COMBO_SELECTION_POLICIES:
        raise ValueError(
            "Unsupported steering.combo_selection_policy="
            f"{policy!r}; expected one of {sorted(ALLOWED_COMBO_SELECTION_POLICIES)}."
        )
    return policy


def _coherence_gate_mode_from_config(config: dict[str, Any]) -> str:
    steering_cfg = config.get("steering", {})
    if not isinstance(steering_cfg, dict):
        steering_cfg = {}
    mode = str(steering_cfg.get("coherence_gate_mode", "absolute_and_relative")).strip().lower()
    if mode not in ALLOWED_COHERENCE_GATE_MODES:
        raise ValueError(
            "Unsupported steering.coherence_gate_mode="
            f"{mode!r}; expected one of {sorted(ALLOWED_COHERENCE_GATE_MODES)}."
        )
    return mode
...
def _build_coherence_summary(
    *,
    ...
    if coherence_gate_mode not in ALLOWED_COHERENCE_GATE_MODES:
        raise ValueError(
            f"coherence_gate_mode must be one of {sorted(ALLOWED_COHERENCE_GATE_MODES)}, got {coherence_gate_mode!r}"
        )
    coherence_baseline_mean = float(np.mean(baseline_scores)) if baseline_scores else 0.0
    coherence_steered_mean = float(np.mean(steered_scores)) if steered_scores else 0.0
    ...
    pass_absolute_and_relative = bool(pass_min_score and pass_max_drop)
    pass_relative_only = bool(pass_max_drop)
    pass_absolute_only = bool(pass_min_score)
    mode_to_pass = {
        "absolute_and_relative": pass_absolute_and_relative,
        "relative_only": pass_relative_only,
        "absolute_only": pass_absolute_only,
    }
    return {
        ...
        "min_steered_score": float(min(steered_scores)) if steered_scores else 0.0,
        ...
        "pass_absolute_and_relative": pass_absolute_and_relative,
        "pass_relative_only": pass_relative_only,
        "pass_absolute_only": pass_absolute_only,
        "gate_mode": coherence_gate_mode,
        "pass": bool(mode_to_pass[coherence_gate_mode]),
    }


def _select_confirm_combo(
    confirm_evaluations: list[dict[str, Any]],
    *,
    min_bidirectional_effect: float,
    combo_selection_policy: str,
) -> tuple[dict[str, Any], dict[str, Any]]:
    if not confirm_evaluations:
        raise ValueError("No confirm evaluations were produced.")
    if combo_selection_policy not in ALLOWED_COMBO_SELECTION_POLICIES:
        raise ValueError(f"Unsupported combo_selection_policy: {combo_selection_policy}")

    def _metric(entry: dict[str, Any]) -> dict[str, Any]:
        metric = entry.get("confirm_metric")
        if not isinstance(metric, dict):
            raise ValueError("Malformed confirm evaluation: missing confirm_metric.")
        return metric

    def _is_eligible(metric: dict[str, Any]) -> bool:
        return bool(
            float(metric["steering_shift_mean"]) > 0.0
            and float(metric["reversal_shift_mean"]) > 0.0
            and float(metric["bidirectional_effect"]) >= min_bidirectional_effect
        )

    fallback_ranked = sorted(
        confirm_evaluations,
        key=lambda entry: (
            float(_metric(entry)["steering_shift_mean"]) > 0.0
            and float(_metric(entry)["reversal_shift_mean"]) > 0.0,
            float(_metric(entry)["bidirectional_effect"]),
            min(
                float(_metric(entry)["steering_shift_mean"]),
                float(_metric(entry)["reversal_shift_mean"]),
            ),
        ),
        reverse=True,
    )
    eligible = [entry for entry in confirm_evaluations if _is_eligible(_metric(entry))]

    if combo_selection_policy == "smallest_feasible_alpha" and eligible:
        selected = sorted(
            eligible,
            key=lambda entry: (
                float(_metric(entry)["alpha"]),
                -float(_metric(entry)["bidirectional_effect"]),
                -min(
                    float(_metric(entry)["steering_shift_mean"]),
                    float(_metric(entry)["reversal_shift_mean"]),
                ),
                int(_metric(entry)["layer"]),
            ),
        )[0]
        return selected, {
            "policy": combo_selection_policy,
            "min_bidirectional_effect_threshold": float(min_bidirectional_effect),
            "eligible_count": len(eligible),
            "fallback_used": False,
            "fallback_reason": None,
        }

    selected = fallback_ranked[0]
    fallback_reason = (
        "no_feasible_combo_meeting_min_bidirectional_effect"
        if combo_selection_policy == "smallest_feasible_alpha" and not eligible
        else "policy_prefers_max_bidirectional_effect"
    )
    return selected, {
        "policy": combo_selection_policy,
        "min_bidirectional_effect_threshold": float(min_bidirectional_effect),
...
@app.function(
    image=image,
    gpu="A100-80GB",
    timeout=WEEK2_UPGRADE_MODAL_TIMEOUT_HOURS * 60 * 60,
    secrets=[
...
def run_trait_validation_remote(
    *,
    ...
    progress_log_every_rows: int,
    progress_log_every_combos: int,
    progress_min_interval_seconds: float,
    resume_from_checkpoint: bool,
    checkpoint_key: str | None,
    checkpoint_write_every_rows: int,
    checkpoint_write_every_combos: int,
    require_capability_available: bool,
    seed_override: int | None = None,
    run_name: str | None = None,
    checkpoint_reload_before_resume: bool = True,
    cross_trait_bleed_enabled: bool = True,
    cross_trait_bleed_traits: list[str] | None = None,
) -> dict[str, Any]:
    import anthropic
    import torch
    import wandb
    ...
    if progress_log_every_rows <= 0:
        raise ValueError("progress_log_every_rows must be > 0.")
    if progress_log_every_combos <= 0:
        raise ValueError("progress_log_every_combos must be > 0.")
    if progress_min_interval_seconds < 0.0:
        raise ValueError("progress_min_interval_seconds must be >= 0.")
    if checkpoint_write_every_rows <= 0:
        raise ValueError("checkpoint_write_every_rows must be > 0.")
    if checkpoint_write_every_combos <= 0:
        raise ValueError("checkpoint_write_every_combos must be > 0.")
    resolved_cross_trait_bleed_traits = (
        [str(x).strip() for x in cross_trait_bleed_traits if str(x).strip()]
        if cross_trait_bleed_traits is not None
        else list(DEFAULT_TRAITS)
    )
    if bool(cross_trait_bleed_enabled):
        if trait not in resolved_cross_trait_bleed_traits:
            raise ValueError(
                f"trait={trait!r} must appear in cross_trait_bleed_traits when cross_trait_bleed_enabled is true."
            )
        if len(resolved_cross_trait_bleed_traits) < 2:
            raise ValueError(
                "cross_trait_bleed_traits must contain at least 2 entries when cross_trait_bleed_enabled is true."
            )
    ...
    run = wandb.init(
        ...
        config={
            ...
            "combo_selection_policy": combo_selection_policy,
            ...
            "cross_trait_bleed_enabled": bool(cross_trait_bleed_enabled),
            "cross_trait_bleed_traits": resolved_cross_trait_bleed_traits,
            ...
            "progress_log_every_rows": progress_log_every_rows,
            "progress_log_every_combos": progress_log_every_combos,
            "progress_min_interval_seconds": progress_min_interval_seconds,
            "resume_from_checkpoint": bool(resume_from_checkpoint),
            "checkpoint_reload_before_resume": bool(checkpoint_reload_before_resume),
            "checkpoint_key": (checkpoint_key or ""),
            "checkpoint_write_every_rows": checkpoint_write_every_rows,
            "checkpoint_write_every_combos": checkpoint_write_every_combos,
            "modal_timeout_hours": WEEK2_UPGRADE_MODAL_TIMEOUT_HOURS,
        },
    )
    ...
    resolved_checkpoint_key = _resolve_checkpoint_key(
        trait=trait,
        seed=seed,
        run_name=run_name,
        checkpoint_key=checkpoint_key,
    )
    checkpoint_path = CHECKPOINT_VOLUME_DIR / f"{resolved_checkpoint_key}.json"
    ...
    if bool(resume_from_checkpoint) and bool(checkpoint_reload_before_resume):
        # Ensure we see latest committed checkpoint state from previous runs.
        vol.reload()
    if bool(resume_from_checkpoint) and checkpoint_path.exists():
        try:
            loaded = json.loads(checkpoint_path.read_text(encoding="utf-8"))
            if (
                loaded.get("trait") == trait
                and int(loaded.get("seed", seed)) == int(seed)
                and str(loaded.get("heldout_hash")) == str(heldout_hash)
            ):
                checkpoint_data = loaded
        except Exception as exc:  # noqa: BLE001
            print(
                f"[week2-upgrade-checkpoint-warning] failed to load checkpoint {checkpoint_path}: {exc}",
                flush=True,
            )

    def persist_checkpoint(stage: str, *, force: bool = False) -> None:
        now_iso = datetime.now(timezone.utc).isoformat()
        checkpoint_data["updated_at"] = now_iso
        checkpoint_data["last_stage"] = stage
        _write_json_atomic(checkpoint_path, checkpoint_data)
        # Modal volumes require explicit commit for cross-container visibility/persistence.
        vol.commit()
        if force:
            run.summary["checkpoint_key"] = resolved_checkpoint_key
            run.summary["checkpoint_path"] = str(checkpoint_path)
            run.summary["checkpoint_last_stage"] = stage
            run.summary["checkpoint_updated_at"] = now_iso

    def emit_progress(stage: str, *, force: bool = False, **details: Any) -> None:
        nonlocal progress_stage_index, last_progress_log_monotonic
        now = time.monotonic()
        if not force and (now - last_progress_log_monotonic) < progress_min_interval_seconds:
            return

        progress_stage_index += 1
        last_progress_log_monotonic = now
        elapsed_minutes = float((now - run_start_monotonic) / 60.0)

        payload = {
            "trait": trait,
            "stage": stage,
            "stage_index": int(progress_stage_index),
            "elapsed_minutes": round(elapsed_minutes, 3),
        }
        payload.update(details)
        print(f"[week2-upgrade-progress] {json.dumps(payload, sort_keys=True)}", flush=True)

        try:
            run.summary["progress_last_stage"] = stage
            run.summary["progress_stage_index"] = int(progress_stage_index)
            run.summary["progress_elapsed_minutes"] = float(elapsed_minutes)
            for key, value in details.items():
                run.summary[f"progress_{key}"] = value

            log_payload: dict[str, float] = {
                f"upgrade/{trait}/progress_stage_index": float(progress_stage_index),
                f"upgrade/{trait}/progress_elapsed_minutes": float(elapsed_minutes),
            }
            completed = details.get("completed")
            total = details.get("total")
            if isinstance(completed, (int, float)) and isinstance(total, (int, float)) and total > 0:
                log_payload[f"upgrade/{trait}/progress_fraction"] = float(completed) / float(total)
            for key, value in details.items():
                if isinstance(value, (int, float)) and np.isfinite(float(value)):
                    log_payload[f"upgrade/{trait}/progress_{key}"] = float(value)
            wandb.log(log_payload)
        except Exception as exc:  # noqa: BLE001
            print(
                f"[week2-upgrade-progress-warning] trait={trait} stage={stage} "
                f"wandb progress logging failed: {exc}",
                flush=True,
            )
    ...
    emit_progress(
        ...
        timeout_hours=WEEK2_UPGRADE_MODAL_TIMEOUT_HOURS,
    )
    persist_checkpoint("run_initialized", force=True)
    ...
    emit_progress("model_loaded", force=True)
    ...
    def make_steering_hook(direction: torch.Tensor, alpha: float):
        def hook_fn(resid_post: torch.Tensor, hook: Any) -> torch.Tensor:
            del hook
            return _apply_steering_direction(resid_post, direction, alpha)
    ...
    persist_checkpoint("split_ready")

    baseline_state = checkpoint_data.get("state", {}).get("baseline", {})
    baseline_low_scores: dict[int, float] = {
        int(k): float(v) for k, v in _decode_int_key_dict(baseline_state.get("baseline_low_scores", {})).items()
    }
    baseline_high_scores: dict[int, float] = {
        int(k): float(v) for k, v in _decode_int_key_dict(baseline_state.get("baseline_high_scores", {})).items()
    }
    baseline_low_responses: dict[int, str] = {
        int(k): str(v) for k, v in _decode_int_key_dict(baseline_state.get("baseline_low_responses", {})).items()
    }
    baseline_high_responses: dict[int, str] = {
        int(k): str(v) for k, v in _decode_int_key_dict(baseline_state.get("baseline_high_responses", {})).items()
    }
    baseline_low_score_rollouts: dict[int, list[float]] = {
        int(k): [float(x) for x in v]
        for k, v in _decode_int_key_dict(baseline_state.get("baseline_low_score_rollouts", {})).items()
        if isinstance(v, list)
    }
    baseline_high_score_rollouts: dict[int, list[float]] = {
        int(k): [float(x) for x in v]
        for k, v in _decode_int_key_dict(baseline_state.get("baseline_high_score_rollouts", {})).items()
        if isinstance(v, list)
    }
    baseline_low_response_rollouts: dict[int, list[str]] = {
        int(k): [str(x) for x in v]
        for k, v in _decode_int_key_dict(baseline_state.get("baseline_low_response_rollouts", {})).items()
        if isinstance(v, list)
    }
    baseline_high_response_rollouts: dict[int, list[str]] = {
        int(k): [str(x) for x in v]
        for k, v in _decode_int_key_dict(baseline_state.get("baseline_high_response_rollouts", {})).items()
        if isinstance(v, list)
    }

    all_eval_rows = sweep_rows + confirm_rows + test_rows
    emit_progress("baseline_start", force=True, total=len(all_eval_rows))
    baseline_done_count = 0
    for baseline_idx, row in enumerate(all_eval_rows, start=1):
        rid = int(row["id"])
        if (
            rid in baseline_low_scores
            and rid in baseline_high_scores
            and rid in baseline_low_responses
            and rid in baseline_high_responses
            and rid in baseline_low_score_rollouts
            and rid in baseline_high_score_rollouts
            and rid in baseline_low_response_rollouts
            and rid in baseline_high_response_rollouts
        ):
            baseline_done_count += 1
            continue
        gt = str(row.get("ground_truth", "N/A"))
        low_prompt = _format_chat_prompt(model.tokenizer, row["system_low"], row["user_query"])
        high_prompt = _format_chat_prompt(model.tokenizer, row["system_high"], row["user_query"])
        ...
        baseline_done_count += 1
        if (
            baseline_idx % progress_log_every_rows == 0
            or baseline_idx == len(all_eval_rows)
        ):
            checkpoint_data["state"]["baseline"] = {
                "baseline_low_scores": _encode_int_key_dict(baseline_low_scores),
                "baseline_high_scores": _encode_int_key_dict(baseline_high_scores),
                "baseline_low_responses": _encode_int_key_dict(baseline_low_responses),
                "baseline_high_responses": _encode_int_key_dict(baseline_high_responses),
                "baseline_low_score_rollouts": _encode_int_key_dict(baseline_low_score_rollouts),
                "baseline_high_score_rollouts": _encode_int_key_dict(baseline_high_score_rollouts),
                "baseline_low_response_rollouts": _encode_int_key_dict(baseline_low_response_rollouts),
                "baseline_high_response_rollouts": _encode_int_key_dict(baseline_high_response_rollouts),
            }
            persist_checkpoint("baseline_progress")
            emit_progress(
                "baseline_progress",
                completed=baseline_done_count,
                total=len(all_eval_rows),
                judge_calls=int(judge_stats[primary_judge_model]["n_calls"]),
    ...
    persist_checkpoint("baseline_complete", force=True)

    sweep_state = checkpoint_data.get("state", {}).get("sweep", {})
    sweep_metric_map: dict[str, SweepMetric] = {}
    for metric_row in sweep_state.get("sweep_metrics", []):
        if not isinstance(metric_row, dict):
            continue
        try:
            metric_obj = SweepMetric(
                layer=int(metric_row["layer"]),
                alpha=float(metric_row["alpha"]),
                steering_shift_mean=float(metric_row["steering_shift_mean"]),
                steering_shift_std=float(metric_row["steering_shift_std"]),
                reversal_shift_mean=float(metric_row["reversal_shift_mean"]),
                reversal_shift_std=float(metric_row["reversal_shift_std"]),
                bidirectional_effect=float(metric_row["bidirectional_effect"]),
                n_prompts=int(metric_row["n_prompts"]),
            )
            sweep_metric_map[_combo_key(metric_obj.layer, metric_obj.alpha)] = metric_obj
        except (KeyError, TypeError, ValueError):
            continue
    sweep_grid: dict[str, list[dict[str, Any]]] = {}
    layer_monotonicity_raw = sweep_state.get("layer_monotonicity", {})
    layer_monotonicity: dict[str, dict[str, float]] = (
        layer_monotonicity_raw if isinstance(layer_monotonicity_raw, dict) else {}
    )
    sweep_total_combos = len(layers) * len(alphas)
    sweep_combo_index = len(sweep_metric_map)
    emit_progress(
        "sweep_start",
        force=True,
        total=sweep_total_combos,
        total_layers=len(layers),
        total_alphas=len(alphas),
    )
    ...
    for layer in layers:
        ...
        for alpha in alphas:
            combo_id = _combo_key(layer, alpha)
            if combo_id in sweep_metric_map:
                metric = sweep_metric_map[combo_id]
            else:
                ...
                sweep_metric_map[combo_id] = metric
                sweep_combo_index = len(sweep_metric_map)
                if (
                    sweep_combo_index % checkpoint_write_every_combos == 0
                    or sweep_combo_index == sweep_total_combos
                ):
                    checkpoint_data["state"]["sweep"] = {
                        "sweep_metrics": [
                            asdict(sweep_metric_map[_combo_key(layer_i, alpha_i)])
                            for layer_i in layers
                            for alpha_i in alphas
                            if _combo_key(layer_i, alpha_i) in sweep_metric_map
                        ],
                        "layer_monotonicity": layer_monotonicity,
                    }
                    persist_checkpoint("sweep_progress")
            layer_rows.append(asdict(metric))
            if (
                len(sweep_metric_map) % progress_log_every_combos == 0
                or len(sweep_metric_map) == sweep_total_combos
            ):
                emit_progress(
                    "sweep_combo_complete",
                    completed=len(sweep_metric_map),
                    total=sweep_total_combos,
                    layer=layer,
                    alpha=alpha,
                    bidirectional_effect=metric.bidirectional_effect,
                    steering_shift=metric.steering_shift_mean,
                    reversal_shift=metric.reversal_shift_mean,
                    judge_calls=int(judge_stats[primary_judge_model]["n_calls"]),
                )

        sweep_grid[str(layer)] = layer_rows

        steering_series = [float(row["steering_shift_mean"]) for row in layer_rows]
        reversal_series = [float(row["reversal_shift_mean"]) for row in layer_rows]
    ...
    sweep_metrics: list[SweepMetric] = [
        sweep_metric_map[_combo_key(layer_i, alpha_i)]
        for layer_i in layers
        for alpha_i in alphas
        if _combo_key(layer_i, alpha_i) in sweep_metric_map
    ]
    if len(sweep_metrics) != sweep_total_combos:
        raise RuntimeError(
            f"Sweep incomplete: expected {sweep_total_combos} combos, got {len(sweep_metrics)}."
        )
    checkpoint_data["state"]["sweep"] = {
        "sweep_metrics": [asdict(x) for x in sweep_metrics],
        "layer_monotonicity": layer_monotonicity,
    }
    persist_checkpoint("sweep_complete", force=True)
    emit_progress("sweep_complete", force=True, completed=sweep_total_combos, total=sweep_total_combos)

    top_combos = _pick_top_combos(sweep_metrics, top_k=confirm_top_k)
    confirm_state = checkpoint_data.get("state", {}).get("confirm", {})
    confirm_map: dict[str, dict[str, Any]] = {}
    for item in confirm_state.get("confirm_evaluations", []):
        if isinstance(item, dict) and isinstance(item.get("combo_key"), str):
            confirm_map[str(item["combo_key"])] = item
    confirm_evaluations: list[dict[str, Any]] = []
    emit_progress("confirm_start", force=True, total=len(top_combos))

    for idx, combo in enumerate(top_combos):
        combo_id = _combo_key(combo.layer, combo.alpha)
        if combo_id in confirm_map:
            evaluation_entry = confirm_map[combo_id]
            evaluation_entry["rank"] = idx + 1
            evaluation_entry["sweep_metric"] = asdict(combo)
        else:
            ...
            confirm_map[combo_id] = evaluation_entry
            if (
                len(confirm_map) % checkpoint_write_every_combos == 0
                or len(confirm_map) == len(top_combos)
            ):
                checkpoint_data["state"]["confirm"] = {
                    "confirm_evaluations": [
                        confirm_map[_combo_key(int(x.layer), float(x.alpha))]
                        for x in top_combos
                        if _combo_key(int(x.layer), float(x.alpha)) in confirm_map
                    ]
                }
                persist_checkpoint("confirm_progress")
        confirm_evaluations.append(evaluation_entry)
        if (
            (idx + 1) % progress_log_every_combos == 0
            or (idx + 1) == len(top_combos)
        ):
            emit_progress(
                "confirm_combo_complete",
                completed=idx + 1,
                total=len(top_combos),
                layer=combo.layer,
                alpha=combo.alpha,
                bidirectional_effect=float(evaluation_entry["confirm_metric"]["bidirectional_effect"]),
                judge_calls=int(judge_stats[primary_judge_model]["n_calls"]),
            )
    checkpoint_data["state"]["confirm"] = {"confirm_evaluations": confirm_evaluations}
    persist_checkpoint("confirm_complete", force=True)

    selected_confirm_eval, confirm_selection_summary = _select_confirm_combo(
        confirm_evaluations,
        min_bidirectional_effect=min_bidirectional_effect,
        combo_selection_policy=combo_selection_policy,
    )
    ...
    emit_progress(
        "confirm_selection_complete",
        force=True,
        selected_layer=selected_layer,
        selected_alpha=selected_alpha,
        eligible_count=float(confirm_selection_summary["eligible_count"]),
        fallback_used=float(confirm_selection_summary["fallback_used"]),
    )
    selected_state = checkpoint_data.get("state", {}).get("selected", {})
    if (
        isinstance(selected_state, dict)
        and int(selected_state.get("layer", -1)) == selected_layer
        and abs(float(selected_state.get("alpha", 0.0)) - selected_alpha) < 1e-9
        and isinstance(selected_state.get("metric"), dict)
        and isinstance(selected_state.get("rows"), list)
    ):
        selected_test_metric = selected_state["metric"]
        selected_rows = selected_state["rows"]
        steering_norm_diagnostics = selected_state.get("steering_norm_diagnostics")
        if not isinstance(steering_norm_diagnostics, dict):
            ...
            checkpoint_data["state"]["selected"]["steering_norm_diagnostics"] = steering_norm_diagnostics
            persist_checkpoint("selected_norm_complete")
    else:
        emit_progress(
            "selected_test_start",
            force=True,
        ...
        emit_progress(
            "selected_test_complete",
            force=True,
            layer=selected_layer,
            alpha=selected_alpha,
            bidirectional_effect=selected_test_metric["bidirectional_effect"],
            judge_calls=int(judge_stats[primary_judge_model]["n_calls"]),
        )
        ...
        checkpoint_data["state"]["selected"] = {
            "layer": selected_layer,
            "alpha": selected_alpha,
        ...
        persist_checkpoint("selected_complete", force=True)

    # Judge reliability calibration on baseline high/low pairs.
    calibration_rows = test_rows[:cross_rater_samples]
    calibration_state = checkpoint_data.get("state", {}).get("calibration", {})
    if (
        isinstance(calibration_state, dict)
        and isinstance(calibration_state.get("summary"), dict)
        and isinstance(calibration_state.get("sheet"), list)
    ):
        calibration_summary = calibration_state["summary"]
        calibration_sheet = calibration_state["sheet"]
    else:
        ...
        emit_progress("judge_calibration_start", force=True, total=len(calibration_rows))
        for calibration_idx, row in enumerate(calibration_rows, start=1):
            rid = int(row["id"])
            gt = str(row.get("ground_truth", "N/A"))
            ...
            if (
                calibration_idx % progress_log_every_rows == 0
                or calibration_idx == len(calibration_rows)
            ):
                emit_progress(
                    "judge_calibration_progress",
                    completed=calibration_idx,
                    total=len(calibration_rows),
                    judge_calls_primary=int(judge_stats[primary_judge_model]["n_calls"]),
                    judge_calls_secondary=int(judge_stats[secondary_judge_model]["n_calls"]),
                )
        ...
        checkpoint_data["state"]["calibration"] = {
            "summary": calibration_summary,
            "sheet": calibration_sheet,
        }
        persist_checkpoint("calibration_complete", force=True)
    emit_progress(
        "judge_calibration_complete",
        force=True,
        kappa=calibration_summary["kappa"],
        pairwise_sign_agreement=calibration_summary["pairwise_sign_agreement"],
        primary_parse_fail_rate=calibration_summary["primary_parse_fail_rate"],
    )

    # Specificity on neutral prompts.
    specificity_state = checkpoint_data.get("state", {}).get("specificity", {})
    if isinstance(specificity_state, dict) and isinstance(specificity_state.get("summary"), dict):
        specificity = specificity_state["summary"]
    else:
        neutral_baseline_scores: list[float] = []
        neutral_steered_scores: list[float] = []
        emit_progress("specificity_start", force=True, total=len(NEUTRAL_USER_PROMPTS))
        for idx, prompt in enumerate(NEUTRAL_USER_PROMPTS):
            ...
            if ((idx + 1) % progress_log_every_rows == 0) or ((idx + 1) == len(NEUTRAL_USER_PROMPTS)):
                emit_progress(
                    "specificity_progress",
                    completed=idx + 1,
                    total=len(NEUTRAL_USER_PROMPTS),
                    judge_calls=int(judge_stats[primary_judge_model]["n_calls"]),
                )
        ...
        checkpoint_data["state"]["specificity"] = {"summary": specificity}
        persist_checkpoint("specificity_complete")
    emit_progress("specificity_complete", force=True, neutral_shift=specificity["neutral_shift"])
    ...
    controls_state = checkpoint_data.get("state", {}).get("controls", {})
    random_control_metrics: list[dict[str, Any]] = []
    if isinstance(controls_state, dict) and isinstance(controls_state.get("random_vectors"), list):
        random_control_metrics = [x for x in controls_state["random_vectors"] if isinstance(x, dict)]
    random_bidirectional: list[float] = [
        float(x.get("bidirectional_effect", 0.0)) for x in random_control_metrics
    ]
    ...
    emit_progress("random_control_start", force=True, total=random_control_vectors)
    for idx in range(random_control_vectors):
        ...
        if idx < len(random_control_metrics):
            continue
        ...
        if (
            (idx + 1) % progress_log_every_combos == 0
            or (idx + 1) == random_control_vectors
        ):
            emit_progress(
                "random_control_progress",
                completed=len(random_control_metrics),
                total=random_control_vectors,
                bidirectional_effect=random_metric_dict["bidirectional_effect"],
            )
        if (
            len(random_control_metrics) % checkpoint_write_every_combos == 0
            or len(random_control_metrics) == random_control_vectors
        ):
            checkpoint_data["state"]["controls"] = {
                "random_vectors": random_control_metrics,
                "shuffled_vectors": controls_state.get("shuffled_vectors", []),
                "random_text_vector": controls_state.get("random_text_vector"),
            }
            persist_checkpoint("controls_random_progress")

    shuffled_control_metrics: list[dict[str, Any]] = []
    if isinstance(controls_state, dict) and isinstance(controls_state.get("shuffled_vectors"), list):
        shuffled_control_metrics = [x for x in controls_state["shuffled_vectors"] if isinstance(x, dict)]
    shuffled_bidirectional: list[float] = [
        float(x.get("bidirectional_effect", 0.0)) for x in shuffled_control_metrics
    ]
    perm_gen = torch.Generator(device="cuda")
    perm_gen.manual_seed(seed + 77219)
    emit_progress("shuffled_control_start", force=True, total=shuffled_control_permutations)
    for idx in range(shuffled_control_permutations):
        perm = torch.randperm(d_model, device="cuda", generator=perm_gen)
        if idx < len(shuffled_control_metrics):
            continue
        ...
        if (
            (idx + 1) % progress_log_every_combos == 0
            or (idx + 1) == shuffled_control_permutations
        ):
            emit_progress(
                "shuffled_control_progress",
                completed=len(shuffled_control_metrics),
                total=shuffled_control_permutations,
                bidirectional_effect=shuffled_metric_dict["bidirectional_effect"],
            )
        if (
            len(shuffled_control_metrics) % checkpoint_write_every_combos == 0
            or len(shuffled_control_metrics) == shuffled_control_permutations
        ):
            checkpoint_data["state"]["controls"] = {
                "random_vectors": random_control_metrics,
            ...
            persist_checkpoint("controls_shuffled_progress")

    random_text_metric = controls_state.get("random_text_vector")
    if not isinstance(random_text_metric, dict):
        hook_name = _hook_name_for_layer(selected_layer)
        hook_filter = {hook_name}
        ...
        random_text_metric = asdict(random_text_eval["metric"])
        checkpoint_data["state"]["controls"] = {
            "random_vectors": random_control_metrics,
            "shuffled_vectors": shuffled_control_metrics,
            "random_text_vector": random_text_metric,
        }
        persist_checkpoint("controls_random_text_complete")
    ...
    controls = {
        ...
        "selected_minus_random_text_bidirectional": float(
            selected_bidirectional - float(random_text_metric.get("bidirectional_effect", 0.0))
        ),
    }
    checkpoint_data["state"]["controls"] = controls
    persist_checkpoint("controls_complete")
    emit_progress(
        "controls_complete",
        force=True,
        selected_minus_random_p95=controls["selected_minus_random_p95_bidirectional"],
        selected_minus_shuffled_p95=controls["selected_minus_shuffled_p95_bidirectional"],
    )

    # Coherence gate on selected combo outputs.
    coherence_state = checkpoint_data.get("state", {}).get("coherence", {})
    if isinstance(coherence_state, dict) and isinstance(coherence_state.get("summary"), dict):
        coherence_summary = coherence_state["summary"]
    else:
        coherence_baseline_scores: list[float] = []
        coherence_steered_scores: list[float] = []
        emit_progress("coherence_start", force=True, total=len(selected_rows))
        for row_idx, row in enumerate(selected_rows, start=1):
            ...
            if (
                row_idx % progress_log_every_rows == 0
                or row_idx == len(selected_rows)
            ):
                emit_progress(
                    "coherence_progress",
                    completed=row_idx,
        ...
        coherence_summary = _build_coherence_summary(
            baseline_scores=coherence_baseline_scores,
            steered_scores=coherence_steered_scores,
            coherence_min_score=coherence_min_score,
            coherence_max_drop=coherence_max_drop,
            coherence_gate_mode=coherence_gate_mode,
        )
        checkpoint_data["state"]["coherence"] = {"summary": coherence_summary}
        persist_checkpoint("coherence_complete")
    emit_progress(
        "coherence_complete",
        force=True,
        coherence_pass=float(coherence_summary["pass"]),
        coherence_drop=coherence_summary["drop_from_baseline"],
    )

    # Cross-trait bleed matrix on selected combo + confirm rows.
    cross_trait_state = checkpoint_data.get("state", {}).get("cross_trait_bleed", {})
    if not bool(cross_trait_bleed_enabled):
        cross_trait_bleed = {}
        cross_trait_bleed_gate = {
            ...
            "required": False,
            "target_trait": trait,
            "configured_traits": resolved_cross_trait_bleed_traits,
            "max_allowed_fraction": float(cross_trait_bleed_max_fraction),
            "pass": True,
            "skip_reason": "disabled_by_caller",
        }
    elif (
        isinstance(cross_trait_state, dict)
        and isinstance(cross_trait_state.get("matrix"), dict)
        and isinstance(cross_trait_state.get("gate"), dict)
    ):
        cross_trait_bleed = cross_trait_state["matrix"]
        cross_trait_bleed_gate = cross_trait_state["gate"]
    else:
        cross_trait_bleed = {}
        emit_progress("cross_trait_bleed_start", force=True, total=len(resolved_cross_trait_bleed_traits))
        for trait_idx, score_trait in enumerate(resolved_cross_trait_bleed_traits, start=1):
            ...
            emit_progress(
                "cross_trait_bleed_progress",
                completed=trait_idx,
                total=len(resolved_cross_trait_bleed_traits),
                score_trait=score_trait,
                abs_effect=abs(float(cross_trait_bleed[score_trait]["bidirectional_effect"])),
            )
        ...
        off_target_effects_abs = {
            ...
            for t in resolved_cross_trait_bleed_traits
            if t != trait
        }
        ...
        checkpoint_data["state"]["cross_trait_bleed"] = {
            "matrix": cross_trait_bleed,
            "gate": cross_trait_bleed_gate,
        }
        persist_checkpoint("cross_trait_bleed_complete")
    emit_progress(
        "cross_trait_bleed_complete",
        force=True,
        off_target_to_target_ratio=cross_trait_bleed_gate.get("off_target_to_target_ratio", 0.0),
        cross_trait_bleed_pass=float(cross_trait_bleed_gate["pass"]),
    )

    # Capability proxy (optional).
    capability_state = checkpoint_data.get("state", {}).get("capability_proxy")
    if isinstance(capability_state, dict) and ("available" in capability_state):
        capability_proxy = capability_state
    else:
        capability_proxy = {"available": False}
        emit_progress("capability_proxy_start", force=True)
        try:
            ...
            for qi, item in enumerate(samples):
                ...
                if _parse_letter(steered_resp) == gold:
                    steered_correct += 1
                if ((qi + 1) % progress_log_every_rows == 0) or ((qi + 1) == len(samples)):
                    emit_progress(
                        "capability_proxy_progress",
                        completed=qi + 1,
                        total=len(samples),
                    )
        ...
        checkpoint_data["state"]["capability_proxy"] = capability_proxy
        persist_checkpoint("capability_proxy_complete")
    emit_progress(
        "capability_proxy_complete",
        force=True,
        capability_available=float(capability_proxy.get("available", False)),
        capability_degradation=float(capability_proxy.get("degradation", 0.0))
        if capability_proxy.get("available", False)
        else 0.0,
    )

    # Hallucination known-fact benchmark check (TruthfulQA generation split).
    truthfulqa_known_fact_state = checkpoint_data.get("state", {}).get("truthfulqa_known_fact")
    truthfulqa_known_fact: dict[str, Any] = {"available": False, "applicable": trait == "hallucination"}
    if trait == "hallucination" and isinstance(truthfulqa_known_fact_state, dict):
        truthfulqa_known_fact = truthfulqa_known_fact_state
    elif trait == "hallucination":
        emit_progress("truthfulqa_known_fact_start", force=True)
        try:
            from datasets import load_dataset
            ...
            for qi, item in enumerate(samples):
                ...
                if ((qi + 1) % progress_log_every_rows == 0) or ((qi + 1) == len(samples)):
                    emit_progress(
                        "truthfulqa_known_fact_progress",
                        completed=qi + 1,
                        total=len(samples),
                    )
        ...
        checkpoint_data["state"]["truthfulqa_known_fact"] = truthfulqa_known_fact
        persist_checkpoint("truthfulqa_known_fact_complete")
        emit_progress(
            "truthfulqa_known_fact_complete",
            force=True,
            truthfulqa_known_fact_available=float(truthfulqa_known_fact.get("available", False)),
            truthfulqa_known_fact_pass=float(truthfulqa_known_fact.get("pass", False))
            if truthfulqa_known_fact.get("available", False)
            else 0.0,
        )

    truthfulqa_objective_state = checkpoint_data.get("state", {}).get("truthfulqa_objective")
    truthfulqa_objective: dict[str, Any] = {"available": False, "applicable": trait == "hallucination"}
    if trait == "hallucination" and isinstance(truthfulqa_objective_state, dict):
        truthfulqa_objective = truthfulqa_objective_state
    elif trait == "hallucination":
        emit_progress("truthfulqa_objective_start", force=True)
        try:
            from datasets import load_dataset
            ...
            for qi, item in enumerate(eval_samples):
                ...
                if minus_letter == gold_letter:
                    minus_correct += 1
                if ((qi + 1) % progress_log_every_rows == 0) or ((qi + 1) == len(eval_samples)):
                    emit_progress(
                        "truthfulqa_objective_progress",
                        completed=qi + 1,
        ...
        checkpoint_data["state"]["truthfulqa_objective"] = truthfulqa_objective
        persist_checkpoint("truthfulqa_objective_complete")
        emit_progress(
            "truthfulqa_objective_complete",
            force=True,
            truthfulqa_objective_available=float(truthfulqa_objective.get("available", False)),
            truthfulqa_objective_pass=float(truthfulqa_objective.get("pass", False))
            if truthfulqa_objective.get("available", False)
            else 0.0,
        )

    primary_parse_fail_rate = float(calibration_summary.get("primary_parse_fail_rate", 1.0))
    score_distribution = {
    ...
    emit_progress(
        ...
        force=True,
        overall_pass=float(quality_gates["overall_pass"]),
        selected_layer=selected_layer,
        selected_alpha=selected_alpha,
        judge_calls=int(judge_stats[primary_judge_model]["n_calls"]),
    )
    ...
    report = {
        ...
        "seed": seed,
        "checkpoint": {
            "key": resolved_checkpoint_key,
            "path": str(checkpoint_path),
            "resume_from_checkpoint": bool(resume_from_checkpoint),
            "last_stage": checkpoint_data.get("last_stage"),
        },
        "layers": layers,
        "alphas": alphas,
        "heldout_hash": heldout_hash,
        ...
        "confirm_selection": confirm_selection_summary,
        ...
        "run_metadata": {
            ...
            "modal_timeout_hours": WEEK2_UPGRADE_MODAL_TIMEOUT_HOURS,
            "checkpointing": {
                "checkpoint_key": resolved_checkpoint_key,
    ...
    wandb.log(
        {
            ...
            f"upgrade/{trait}/selected_confirm_fallback_used": float(confirm_selection_summary["fallback_used"]),
            f"upgrade/{trait}/selected_confirm_eligible_count": float(confirm_selection_summary["eligible_count"]),
            ...
            f"upgrade/{trait}/cross_trait_bleed_ratio": cross_trait_bleed_gate.get("off_target_to_target_ratio", 0.0),
            f"upgrade/{trait}/overall_pass": float(quality_gates["overall_pass"]),
        }
    )
    emit_progress("wandb_metrics_logged", force=True)
    ...
    checkpoint_data["state"]["final_report_digest"] = {
        "selected_layer": selected_layer,
        "selected_alpha": selected_alpha,
        "overall_pass": bool(quality_gates["overall_pass"]),
        "modal_report_path": str(modal_report_path),
    }
    persist_checkpoint("final_report_written", force=True)
    emit_progress(
        "artifact_logged",
        force=True,
        modal_report_path=str(modal_report_path),
...
def fetch_checkpoint_remote(*, checkpoint_key: str) -> dict[str, Any]:
    _set_modal_cache_env()
    # Ensure this reader sees latest committed checkpoint files.
    vol.reload()
    resolved = _resolve_checkpoint_key(
        trait="unknown",
        seed=0,
        run_name=None,
        checkpoint_key=checkpoint_key,
    )
    checkpoint_path = CHECKPOINT_VOLUME_DIR / f"{resolved}.json"
    if not checkpoint_path.exists():
        return {
            "checkpoint_key": resolved,
            "exists": False,
            "checkpoint_path": str(checkpoint_path),
            "fetched_at": datetime.now(timezone.utc).isoformat(),
        }
    payload = json.loads(checkpoint_path.read_text(encoding="utf-8"))
    payload["checkpoint_key"] = resolved
    payload["exists"] = True
...
def _local_spot_check(
    *,
    ...
    def hook_fn(resid_post: torch.Tensor, hook: Any) -> torch.Tensor:
        del hook
        return _apply_steering_direction(
            resid_post,
            direction.to(resid_post.dtype),
            alpha,
        )
    ...
    with model.hooks(fwd_hooks=[(_hook_name_for_layer(layer), hook_fn)]):
...
def main(
    ...
    progress_log_every_rows: int = WEEK2_UPGRADE_PROGRESS_LOG_EVERY_ROWS,
    progress_log_every_combos: int = WEEK2_UPGRADE_PROGRESS_LOG_EVERY_COMBOS,
    progress_min_interval_seconds: float = WEEK2_UPGRADE_PROGRESS_MIN_INTERVAL_SECONDS,
    resume_from_checkpoint: bool = True,
    checkpoint_key: str = "",
    checkpoint_write_every_rows: int = 3,
    checkpoint_write_every_combos: int = 1,
    fetch_checkpoint_key: str = "",
    ...
    if fetch_checkpoint_key.strip():
        payload = fetch_checkpoint_remote.remote(checkpoint_key=fetch_checkpoint_key.strip())
        out_path = (
            Path(checkpoint_output_path)
            if checkpoint_output_path.strip()
            else (
                RESULTS_DIR
                / "checkpoints"
                / f"week2_upgrade_checkpoint_{fetch_checkpoint_key.strip()}_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}.json"
            )
        )
        ...
        print(
            json.dumps(
                {
                    "checkpoint_key": payload.get("checkpoint_key", fetch_checkpoint_key.strip()),
    ...
    alpha_values = (
        _normalize_csv_floats(alpha_grid)
        if alpha_grid.strip()
        else _alpha_grid_from_config(config)
    )
    resolved_combo_selection_policy = (
        combo_selection_policy.strip().lower()
        if combo_selection_policy.strip()
        else _combo_selection_policy_from_config(config)
    )
    if resolved_combo_selection_policy not in ALLOWED_COMBO_SELECTION_POLICIES:
        raise ValueError(
            f"combo_selection_policy must be one of {sorted(ALLOWED_COMBO_SELECTION_POLICIES)}, "
            f"got {resolved_combo_selection_policy!r}"
        )
    resolved_coherence_gate_mode = (
        coherence_gate_mode.strip().lower()
        if coherence_gate_mode.strip()
        else _coherence_gate_mode_from_config(config)
    )
    if resolved_coherence_gate_mode not in ALLOWED_COHERENCE_GATE_MODES:
        raise ValueError(
            f"coherence_gate_mode must be one of {sorted(ALLOWED_COHERENCE_GATE_MODES)}, "
            f"got {resolved_coherence_gate_mode!r}"
        )
    ...
    result = run_trait_validation_remote.remote(
        ...
        judge_directionality_threshold=judge_directionality_threshold,
        min_bidirectional_effect=min_bidirectional_effect,
        combo_selection_policy=resolved_combo_selection_policy,
        control_test_max_score=control_test_max_score,
        specificity_max_abs_shift=specificity_max_abs_shift,
        truthfulqa_samples=truthfulqa_samples,
        truthfulqa_min_plus_minus_delta=truthfulqa_min_plus_minus_delta,
        ...
        coherence_min_score=coherence_min_score,
        coherence_max_drop=coherence_max_drop,
        coherence_gate_mode=resolved_coherence_gate_mode,
        cross_trait_bleed_max_fraction=cross_trait_bleed_max_fraction,
        judge_rpm_limit_per_run=judge_rpm_limit_per_run,
        judge_min_interval_seconds=judge_min_interval_seconds,
        judge_global_rpm_budget=judge_global_rpm_budget,
        ...
        checkpoint_key=(checkpoint_key.strip() or None),
        checkpoint_write_every_rows=checkpoint_write_every_rows,
        checkpoint_write_every_combos=checkpoint_write_every_combos,
        ...
        json.dumps(
            {
                ...
                "combo_selection_policy": result["report"]["confirm_selection"]["policy"],
