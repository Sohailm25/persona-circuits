"""Generate extraction prompt datasets for trait_lanes_v2 without mutating legacy Week 1 files."""
...
random.seed(SEED_EXTRACTION)
np.random.seed(SEED_EXTRACTION)
...
def _generate_lane_queries(
    ...
    *,
    ...
    template_id = str(lane_cfg["prompt_generator_template"])
    if target_total is None:
        specs = resolve_template_specs(template_id, mode="extraction")
    else:
        specs = allocate_category_specs(template_id, mode="extraction", target_total=int(target_total))
    seen = set(blocked)
    records = []
    idx = 0
    for spec in specs:
        ...
        if len(valid_items) < target_n:
            raise RuntimeError(
                f"Insufficient valid extraction prompts for lane={lane_id} category={spec.category} "
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Optional override for extraction pairs per selected lane. Uses proportional category allocation.",
    )
    ...
    if args.dry_run_plan:
        ...
        packet.update({
            ...
            "notes": [
                ...
                "Legacy Week 1/2 prompt files remain untouched.",
            ],
        })
        ...
        if args.output_json:
            ...
            out_path = TRAIT_LANE_RESULTS_DIR / f"week2_trait_lane_extraction_generation_plan_{timestamp}.json"
    ...
    blocked = load_blocked_queries(
        exclude_paths={planned_output_path(lane_id=lane_id, mode="extraction", output_suffix=args.output_suffix) for lane_id in lane_ids}
    )
    summary = {"timestamp_utc": datetime.now(timezone.utc).isoformat(), "mode": "extraction", "lanes": {}}
    for lane_id in lane_ids:
        ...
        out_path = planned_output_path(lane_id=lane_id, mode="extraction", output_suffix=args.output_suffix)
    ...
    TRAIT_LANE_RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    summary_path = TRAIT_LANE_RESULTS_DIR / f"week2_trait_lane_extraction_generation_summary_{timestamp}.json"
